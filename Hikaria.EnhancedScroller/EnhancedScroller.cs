...
namespace Hikaria.ES
{
    ...
    public class EnhancedScroller : MonoBehaviour
    {
            ...
            )
        {
            ...
            // start tweening
            StartCoroutine(TweenPosition(tweenType, tweenTime, ScrollPosition, newScrollPosition, jumpComplete, forceCalculateRange).WrapToIl2Cpp());
        }
        ...
        private void AddEventTriggerEntry(EventTrigger trigger, EventTriggerType eventType, Action<PointerEventData> callback)
        {
            var entry = new EventTrigger.Entry();
            entry.eventID = eventType;
            entry.callback.AddListener((UnityAction<BaseEventData>)((data) => { callback(data.Cast<PointerEventData>()); }));
        ...
        void OnEnable()
        {
            ...
            _scrollRect.onValueChanged.AddListener((UnityAction<Vector2>)_ScrollRect_OnValueChanged);
        }

        void OnDisable()
        {
            // when the scroller is disabled, remove the listener
            _scrollRect.onValueChanged.RemoveListener((UnityAction<Vector2>)_ScrollRect_OnValueChanged);
        }
